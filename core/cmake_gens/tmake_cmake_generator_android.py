...
class CMakeGeneratorAndroid(CMakeGenerator):
    """cmake generator cmake class for android"""

    def __init__(self, arch):
        CMakeGenerator.__init__(self, arch)
    ...
    def generate_global_common(self):
        ...
        if core.data.arguments.has_opt("--android_lto"):
    ...
    def create_build_vars(self):
        ...
        if "CMAKE_SYSTEM_VERSION" not in self.info.build_vars:
            if self.arch in core.TARGET_ANDROID_CPU_64_ALL:
        ...
        if core.data.build_config == core.CONFIG_DEBUG and core.data.arguments.tmake_cmd() == "build":
            ...
            if ndk_version:
                ...
                core.e('!!!!!!!!ndk version failed to get !!!!!!!! ')
        else:
            if has_android_stl:
...
def cmake_plugin_init(arch):
    """cmake plugin entry"""
    return CMakeGeneratorAndroid(arch)
