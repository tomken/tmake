...
import lib
from lib.utils.tmake_cmake import *
...
class CommandProject(lib.Command):
    ...
    def run(self):
        arch_list = lib.get_archs()
        self.param_check()

        for arch in arch_list:
            # fusion类型的生成project跳过
            if arch == lib.TARGET_CPU_FUSION:
                continue

            # 生成CMakeList
            try:
                acg_list = general_cmake_info(arch, False)
            except lib.SkipException:
                continue
            ...
            if not lib.data.use_cmakelist:
                ...
                lib.write_entire_file(cmake_path, all_text)
    ...
    def param_check(self):
        ...
        if len(argv) > 0:
            if argv[0] not in self.supported_list:
                lib.i(self.help())
                raise lib.TmakeException('{} is error, must be in {}'.format(argv[0], self.supported_list))
        else:
            lib.i(self.help())
            raise lib.TmakeException('project failed')
        self.param_ide_name = argv[0]
        lib.data.environment.update_vs_tool_name(self.param_ide_name)
