...
class AbtorCMakeGeneratorLinux(AbtorCMakeGenerator):
    ...
    def generate_global_common(self):
        ...
        # if self.arch == abtor.TARGET_CPU_X86 and PlatformInfo.is_64bit():
        #     self.info.global_c_flags += ' -m32 '
        #     self.info.global_cxx_flags += ' -m32 '
        # elif self.arch == abtor.TARGET_CPU_X64 and not PlatformInfo.is_64bit():
    ...
    def generate_module_common(self, module):
        ...
        if stdafx_path:
            ...
            cmake_text += "SET(GCH_CXX_FLAGS ${CMAKE_CXX_FLAGS})" + abtor.LINESEP + abtor.LINESEP
...
"""
            cmake_text += "    target_compile_options({} PRIVATE -Winvalid-pch -include stdafx.h){}".format(module.name,
                                                                                                        abtor.LINESEP)
            cmake_text += "    ADD_CUSTOM_COMMAND(OUTPUT " + stdafx_path + ".gch COMMAND ${CCACHE_PROGRAM} ${CMAKE_CXX_COMPILER} ${GCH_CXX_FLAGS} " + stdafx_path + ")" + abtor.LINESEP
            cmake_text += "    ADD_CUSTOM_TARGET({}_gch DEPENDS {}.gch){}".format(module.name, stdafx_path, abtor.LINESEP)
            cmake_text += "    ADD_DEPENDENCIES({} {}_gch){}".format(module.name, module.name, abtor.LINESEP)
            cmake_text += "ENDIF()" + abtor.LINESEP + abtor.LINESEP
        ...
        return AbtorCMakeGenerator.generate_module_common(self, module) + cmake_text


def cmake_plugin_init(ctx):
    """cmake plugin entry"""
    return AbtorCMakeGeneratorLinux(ctx)
