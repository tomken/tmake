...
调试宝典：
1、修改 cmake_helper.py， 注释掉 abtor.write_entire_file，直接修改原来的CMakeLists.txt查看效果。
2、https://cmake.org/cmake/help/v3.5/prop_tgt/RESOURCE.html 在cmake官网搜索可用的方式。
"""
import os

import lib
from lib import CMAKE_CACHE_TEMPLATE
from lib import CMAKE_GLOBAL_TEMPLATE
from lib import CMAKE_HEADER_TEMPLATE
from lib import PlatformInfo
from lib.abtor_cmakelists import recover_cmakelists, copy_bin_to_export, move_header_files, arrange_dir, \
    delete_empty_dir, copy_libs_to_export, change_cmakelists_output
from lib.info.abtor_builtin import abtor_path, abtor_glob
from lib.info.abtor_path_info import AbtorPathInfo
from lib.utils import tmake_utils
from multiprocessing import cpu_count

from lib.utils import process_utils
...
class CMakeGenerator(object):
    ...
    def __init__(self, arch):
        self.arch = arch
        self.path = AbtorPathInfo(arch, abtor.data.current_project.folder)
        self.info = abtor.CMakeProjectInfo()
        self.info.parse(abtor.data.current_project, self.path)
        self.cmake_text = ""
    ...
    def generate(self):
        ...
        if self.info.cmake_command:
            self.cmake_text += self.info.cmake_command + abtor.LINESEP + abtor.LINESEP
        ...
        for app in self.info.apps:
            if self.info.build_target == abtor.PLATFORM_ANDROID:
                app.link_style = abtor.CXX_LIBRARY_LINK_STYLE_SHARED
    ...
    def __generate_header(self):
        ...
        # 设置ccache缓存相关内容
        cache_path = ""
        use_cache = not abtor.data.arguments.has_opt("--no_cache") and not PlatformInfo.is_windows_system()
        if use_cache:
            ...
            if cache_result:
                ...
                use_cache = False
            if not use_cache:
                abtor.log.i("enable ccache, but ccache not install!!!")
        abtor_path = abtor.data.arguments.abtor_path()
        self.cmake_text += CMAKE_HEADER_TEMPLATE.format(
            self.info.project_name,
            self.info.mini_version,
            abtor.BUILD_OUTPUT_NAME,  # bin
            abtor.BUILD_INSTALL_PREFIX,  # export
            self.info.build_target.upper(),
            self.info.build_config.upper(),
            CMAKE_CACHE_TEMPLATE.format(cache_path, abtor_path, abtor_path) if use_cache else ""
        )
    ...
    def __generate_global(self):
        ...
        self.info.global_c_flags += abtor.data.current_project.get_feature_headers_flags()
        self.info.global_cxx_flags += abtor.data.current_project.get_feature_headers_flags()
        ...
        self.cmake_text += CMAKE_GLOBAL_TEMPLATE.format(
            ...
            comm_utils.flat_ccxx_defines(self.info.global_defines, True),
        ...
        if self.info.build_target in [abtor.PLATFORM_MAC, abtor.PLATFORM_IOS]:
    ...
    def __generate_resource(self, resource):
        ...
        for bundle in resource.bundles:
            ...
            if PlatformInfo.is_windows_system() or ".xcassets" in bundle_path:
                ...
                clean_bundle = True
                comm_utils.copyFiles(abtor_path(bundle), link_path)
                #comm_utils.set_symlink(abtor_path(bundle), link_path)
    ...
    def __generate_module(self, module):
        ...
        module_info += module.pre_cmake_command + abtor.LINESEP
        module_info += module.post_cmake_command + abtor.LINESEP
    ...
    def __generate_external_build(self, module):
        ...
        """
        module_info = ""
        module_info += "ADD_SUBDIRECTORY({} {})".format(module.path, module.path) + abtor.LINESEP
        self.cmake_text += module_info

        if abtor.data.arguments.abtor_cmd() == "project":
            path_info = AbtorPathInfo(self.arch, abtor.data.arguments.work_path())
            base_path = path_info.project_path
        else:
            base_path = abtor.data.project.get_build_folder(self.arch)
        export_path = os.path.join(base_path, abtor.BUILD_INSTALL_PREFIX, module.name)
        exectable_output_path = os.path.join(base_path, abtor.BUILD_OUTPUT_NAME, module.name)
        library_output_path = os.path.join(base_path, abtor.BUILD_OUTPUT_NAME, module.name)
    ...
    def __generate_module_qt(self, module):
        ...
        ret_str = abtor.CMAKE_QT_MODULE_TEMPLATE.format(qt_components, module.name, qt_moc_headers, qt_ui)
        return ret_str
    ...
    def __generate_module_link_flags(self, module):
        ...
        if module.link_style == abtor.CXX_LIBRARY_LINK_STYLE_SHARED:
            link_flags_key = "CMAKE_SHARED_LINKER_FLAGS"
        elif module.link_style == abtor.CXX_LIBRARY_LINK_STYLE_STATIC:
            link_flags_key = "CMAKE_STATIC_LINKER_FLAGS"
        info = ""
        # 目前只给android的so类型添加、非project命令 添加特定的linker_flags，这样有未找到的符号编译so不通过
        if module.link_style == abtor.CXX_LIBRARY_LINK_STYLE_SHARED \
                and self.info.build_target == abtor.PLATFORM_ANDROID \
        ...
        if abtor.data.arguments.has_opt("--link_map") \
                and module.link_style == abtor.CXX_LIBRARY_LINK_STYLE_SHARED:
                    if PlatformInfo.is_windows_system():
                        windows_flag = '\"${CMAKE_SHARED_LINKER_FLAGS}'
                        info += 'set(CMAKE_SHARED_LINKER_FLAGS {} /MAP:{}.map\") {}'.format(windows_flag, module.name, abtor.LINESEP)
                    else:
                        module.linker_flags += '-Wl,-Map,{}.map'.format(module.name)
        # ios的 framework 默认添加下面的flags，为了一些依赖不打入库中也不报符号找不到错误。
        if module.link_style == abtor.CXX_LIBRARY_LINK_STYLE_FRAMEWORK \
                and self.info.build_target == abtor.PLATFORM_IOS:
            module.linker_flags += " -all_load -undefined dynamic_lookup "
        if module.linker_flags:
            # info += "SET({} \"${{{}}} {}\"){}" \
            #     .format(link_flags_key, link_flags_key, module.linker_flags, abtor.LINESEP)
    ...
    def __generate_module_install(self, module):
        ...
        return "INSTALL (TARGETS {} DESTINATION {}){}" \
            .format(module.name, abtor.BUILD_OUTPUT_NAME, abtor.LINESEP)
    ...
    def __generate_module_link_lib(self, module):
        ...
        """
        framework_info = ""
        if abtor.data.target in [abtor.PLATFORM_IOS, abtor.PLATFORM_MAC] and module.frameworks:
        ...
        for item in module.deps:
            if module.link_all_symbol_libs and item in module.link_all_symbol_libs:
                if abtor.data.target in [abtor.PLATFORM_IOS, abtor.PLATFORM_MAC]:
    ...
    def __generate_module_properties(self, module):
        ...
        if module.properties:
            ...
            result = "SET_TARGET_PROPERTIES({} PROPERTIES {}){}".format(module.name, info, abtor.LINESEP)
        return result
    ...
    def __generate_module_srcs(self, module):
        ...
        if module.link_style in (abtor.CXX_LIBRARY_LINK_STYLE_STATIC, abtor.CXX_LIBRARY_LINK_STYLE_SHARED,
                                 "MODULE", abtor.CXX_LIBRARY_LINK_STYLE_FRAMEWORK):
            add_key = "ADD_LIBRARY"
            if self.info.build_target in [abtor.PLATFORM_MAC, abtor.PLATFORM_IOS]\
            ...
            # if abtor.PLATFORM_WINDOWS == abtor.data.platform.get_host():
            #     app_type = 'WIN32'
            # else:
            #     app_type =''
        ...
        if module.link_style == abtor.CXX_LIBRARY_LINK_STYLE_FRAMEWORK:
            library_style = abtor.CXX_LIBRARY_LINK_STYLE_SHARED
        add_cmd =  "{}({} {} {} {} {} {} {} {}){}" \
            .format(add_key,
                    module.name,
                    xctest_cmd,
    ...
    def generate_module_common(self, module):
        ...
        """
        cmake_text = ""
        if (self.info.is_project_cmd and abtor.PLATFORM_WINDOWS == abtor.data.platform.get_host()) or (
                "wince" in self.arch and abtor.PLATFORM_WINDOWS == abtor.data.platform.get_host()):
            ...
            if len(module.cxx_flags):
                ...
                for src in module.srcs:
                    if src.endswith("stdafx.cpp"):
                        print(src)
                        cmake_text += abtor.LINESEP + "# Precompile Compile Header" + abtor.LINESEP
                        cmake_text += "target_compile_options({} PRIVATE /Yu){}".format(module.name, abtor.LINESEP)
                        cmake_text += "SET(PrecompiledBinary ${CMAKE_CURRENT_BINARY_DIR}/" + module.name + ".dir/${CMAKE_BUILD_TYPE}/" + module.name + ".pch)" + abtor.LINESEP
                        cmake_text += "SET_SOURCE_FILES_PROPERTIES(" + src + """ PROPERTIES COMPILE_FLAGS "/Yc\\"stdafx.h\\" /Fp\\"${PrecompiledBinary}\\"" OBJECT_OUTPUTS "${PrecompiledBinary}")""" + abtor.LINESEP + abtor.LINESEP
                        break
        else:
            if len(module.c_flags):
                cmake_text += "target_compile_options({} PRIVATE $<$<COMPILE_LANGUAGE:C>:{}>){}". \
                    format(module.name, module.c_flags, abtor.LINESEP)
            if len(module.cxx_flags):
                cmake_text += "target_compile_options({} PRIVATE $<$<COMPILE_LANGUAGE:CXX>:{}>){}". \
                    format(module.name, module.cxx_flags, abtor.LINESEP)

        # 如果有资源，则指定资源信息
        if module.name in self.resources:
            cmake_text += abtor.CMAKE_MODULE_ADD_RES_TEMPLATE.format(module.name,
        ...
        if self.info.is_project_cmd:
            ...
            # 针对是否头文件和源文件分开做的处理
            is_together = abtor.data.arguments.has_opt("--together")
    ...
    def __module_deps_headers(self, module):
        ...
        for item in set(module.include_dirs):
            full_path = abtor_path(item)
            if full_path.startswith(abtor.ABTOR_LIBRARIES_PATH):
                temp_list = abtor_glob(full_path, ["*.h", "*.hpp"], True)
    ...
    def make_project(self, cmake_list_path, name):
        ...
        target = abtor.data.target
        project_folder = self.path.project_path

        command_text = ""
        use_nmake = ""
        if PlatformInfo.is_windows_system():
            vs_tools = abtor.data.environment.get_vs_tool_path(self.arch)
            if target != abtor.PLATFORM_ANDROID and target != abtor.PLATFORM_WINDOWS:
                raise abtor.AbtorException('unsupported target : ' + target)
        ...
        if abtor.data.use_cmakelist:
            if abtor.data.arguments.abtor_cmd() == "project":
                ...
                base_path = abtor.data.project.get_build_folder(self.arch)
            export_path = os.path.join(base_path, abtor.BUILD_INSTALL_PREFIX)
            exectable_output_path = os.path.join(base_path, abtor.BUILD_OUTPUT_NAME)
            library_output_path = os.path.join(base_path, abtor.BUILD_OUTPUT_NAME)
            ...
            #
            # ret = abtor.subprocess.call(pre_command_text, shell=not PlatformInfo.is_windows_system())
            # if ret != 0:
            #     raise abtor.AbtorException('Set CMAKE_INSTALL_PREFIX failed! return code is {}'.format(ret))

        ret = abtor.subprocess.call(command_text, shell=not PlatformInfo.is_windows_system())
        if abtor.data.use_cmakelist:
            recover_cmakelists(self.path.project_folder)

        if abtor.data.use_abtorProj_cmakelist:
            for external_build in self.info.external_builds:
        ...
        if ret != 0:
            raise abtor.AbtorException('build failed! return code is {}'.format(ret))
    ...
    def get_cmake_generator_name(self, project_type):
        ...
        """
        arc = ''
        if self.arch.lower().endswith(abtor.TARGET_CPU_X64):
            arc = ' Win64'
        if self.arch.lower().endswith(abtor.TARGET_CPU_WINCE_ARMV4I):
            arc = ' ' + abtor.WINCE_CPU_MAP[self.arch]
        if self.arch.lower().endswith(abtor.TARGET_CPU_WINCE_ARMV7):
            arc = ' ' + abtor.WINCE_CPU_MAP[self.arch]
        if self.arch.lower().endswith(abtor.TARGET_CPU_WINCE_CE6_ARMV4I):
            arc = ' ' + abtor.WINCE_CPU_MAP[self.arch]
    ...
    def __build_params(self):
        ...
        """
        command_text = ""
        self.info.build_vars["CMAKE_BUILD_TYPE"] = abtor.data.build_config.capitalize()
    ...
    def __build_target(self):
        ...
        if target == abtor.PLATFORM_WINDOWS:
            command_text += ' "' + self.cmake_home + '" --build . --target "install" '
            if not abtor.data.use_cmakelist:
                command_text += self.get_mp_build_string()
        elif target == abtor.PLATFORM_IOS:
    ...
    def get_mp_build_string(self):
        mp_args = ""
        if abtor.data.arguments.has_flag("nmp", "nmp"):
            return mp_args
    ...
    def run_build(self):
        ...
        if PlatformInfo.is_windows_system():
            ...
            if not abtor.data.arguments.has_flag("nmp",
                                                 "nmp") and target == abtor.PLATFORM_WINDOWS and not "wince" in self.arch\
                                                and not abtor.data.use_cmakelist:
                use_nmake = '-G "NMake Makefiles JOM" '
                jom_exe = os.path.join(abtor.data.arguments.abtor_path(), "tools", "jom.exe")
                use_nmake += ' -DCMAKE_MAKE_PROGRAM="' + jom_exe + '"'
                abtor.log.i("use_nmake = " + use_nmake)
            elif target == abtor.PLATFORM_WINDOWS and "wince" in self.arch:
                use_nmake = '-G "Visual Studio 9 2008 ' + abtor.WINCE_CPU_MAP[self.arch] + '"'
            else:
        ...
        if target == abtor.PLATFORM_WINDOWS and "wince" in self.arch:
            configuration = ''
            if self.info.build_config == abtor.CONFIG_DEBUG:
                configuration = 'Debug'
            elif self.info.build_config == abtor.CONFIG_RELWITHDEBINFO:
                configuration = 'RelWithDebInfo'
            else:
                configuration = 'Release'
            command_text += "&&"
            command_text += 'msbuild.exe ' + self.info.project_name + ".sln " + '/t:ReBuild ' + '/p:Configuration=' + configuration + ' /p:Platform=' + '"' + \
                            abtor.WINCE_CPU_MAP[self.arch] + '"'
        if abtor.data.use_cmakelist and target != abtor.PLATFORM_WINDOWS:
            if abtor.data.arguments.abtor_cmd() == "project":
                path_info = AbtorPathInfo(self.arch, abtor.data.arguments.work_path())
                base_path = path_info.project_path
            ...
            ret = abtor.subprocess.call(pre_command_text, shell=True)
            if ret != 0:
                raise abtor.AbtorException('Set CMAKE_INSTALL_PREFIX failed! return code is {}'.format(ret))
    ...
    def execute_build_command(self, command_text):
        ...
        #ret = abtor.subprocess.call(command, shell=True)
        ret = abtor.subprocess.call(command, shell=True)
        if ret != 0:
            raise abtor.AbtorException('build failed! return code is {}'.format(ret))
        if abtor.data.use_cmakelist or abtor.data.use_abtorProj_cmakelist:
            if abtor.data.use_cmakelist:
                recover_cmakelists(self.path.project_folder)
            if abtor.data.arguments.abtor_cmd() == "project":
                src_dir = os.path.join(self.path.project_path, abtor.BUILD_OUTPUT_NAME)
            else:
                src_dir = os.path.join(self.path.build_path, abtor.BUILD_OUTPUT_NAME)
            # build folder
            arrange_dir(src_dir, src_dir)
            delete_empty_dir(src_dir)
            # export folder
            dst_dir = os.path.join(self.path.build_path, abtor.BUILD_INSTALL_PREFIX, abtor.BUILD_OUTPUT_NAME)
            copy_libs_to_export(src_dir, dst_dir)
            src_dir = os.path.join(self.path.build_path, abtor.BUILD_INSTALL_PREFIX)
    ...
    def __do_rm_build_bin_dir(self):
        ...
        for dir in rm_dirs:
            abtor.log.v("do_rm_build_bin_dir: " + dir)
    ...
    def __generate_module_script_tasks(self, module):
        ...
        for (cond, task) in module.tasks.items():
            ...
            # python D:/workspace/abtor/abtor/tester/single_binary\abtor_entry.python
            # D:/workspace/abtor/abtor/ task single_binary
            # 这里执行task的时候会多拼接上一遍，不过没影响，因为执行的task一般不进行build操作。
            arguments_info = abtor.data.arguments.clone(["task", module.name, cond, "-a", self.arch])
            arguments_str = ""
            ...
            command = "python {} {} {}".format(
                ...
                arguments_str)
            abtor.log.i(command)
            if cond == abtor.SCRIPT_PRE_BUILD:
                cmake_text += abtor.CMAKE_MODULE_PRE_BUILD_TEMPLATE.format(fake_target,
                                                                           command,
                                                                           module.name)
            else:
                cmake_text += abtor.CMAKE_MODULE_PRE_LINK_OR_POST_BUILD_TEMPLATE.format(module.name,
                                                                                        str.upper(cond),
                                                                                        command)
        return cmake_text
