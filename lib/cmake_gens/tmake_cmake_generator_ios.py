...
class AbtorCMakeGeneratorIOS(AbtorCMakeGenerator):
    """cmake generator cmake class for iOS"""

    def generate_global_common(self):
        if self.info.build_config == abtor.CONFIG_RELEASE:
            ...
            if remove_default_flags == False:
                for item in (self.info.binaries, self.info.libraries, self.info.apps):
                    for library in item:
                        ...
                        elif self.arch == abtor.TARGET_CPU_SIMULATOR:
    ...
    def generate_module_common(self, module):
        ...
        if abtor.data.arguments.has_opt("--ios_no_symbol"):
            has_symbol = "NO"

        module_info = XCODE_TARGET_PROPERTIES_TPL.format(module.name,
                                                         "XCODE_ATTRIBUTE_GCC_GENERATE_DEBUGGING_SYMBOLS",
                                                         has_symbol)
        if has_symbol == "YES" and abtor.data.build_config == abtor.CONFIG_RELEASE:
            module_info += XCODE_TARGET_PROPERTIES_TPL.format(module.name,
        ...
        if module.link_style == abtor.CXX_LIBRARY_LINK_STYLE_FRAMEWORK:
            ...
            bundle_id = "com.amap.abtor.framework"
        ...
        if module.launch_image:
            module_info += XCODE_TARGET_PROPERTIES_TPL.format(module.name,
                                                              ...
                                                              module.launch_image)
        return module_info + AbtorCMakeGenerator.generate_module_common(self, module)
    ...
    def run_build(self):
        if self.arch == abtor.TARGET_CPU_FUSION:
            ...
            return AbtorCMakeGenerator.run_build(self)
    ...
    def generate_cmake_lipo_text(self):
        ...
        for module in self.info.libraries:
            ...
            for call_function, name in params_dict.items():
                os_library_path = os.path.join(call_function(abtor.TARGET_CPU_OS), lib_file_name)
                simulator_library_path = os.path.join(call_function(abtor.TARGET_CPU_SIMULATOR), lib_file_name)
                fusion_library_path = os.path.join(call_function(abtor.TARGET_CPU_FUSION), lib_file_name)
                cmd = ""
                if module.link_style == abtor.CXX_LIBRARY_LINK_STYLE_FRAMEWORK:
                    ...
                    #     cmd += "\nCOMMAND cp -rf {} {}" \
                    #         .format(os.path.join(call_function(abtor.TARGET_CPU_OS), lib_file_name_list[1]),
                    #                 call_function(abtor.TARGET_CPU_FUSION))
...
def cmake_plugin_init(ctx):
    """cmake plugin entry"""
    return AbtorCMakeGeneratorIOS(ctx)
