...
class ProjectInfo(object):
    ...
    def __parse_ci_config_aop_info(self, project_path):
        ...
        :return:
        """
        lib.i("parse aop config... {}".format(project_path))
        ...
        if not os.path.exists(ci_config_file_path):
            lib.e("{} not found in {}, skip!".format(self.ci_config_file, project_path))
            return
        ...
        try:
            ...
            traceback.print_exc()
            raise lib.TmakeException("parse [{}] error, {}".format(self.ci_config_file, repr(e)))

        if ProjectInfo.AOP_CONFIG_KEY_WORD in ci_config_dict:
            self.aop_config_dict = ci_config_dict[ProjectInfo.AOP_CONFIG_KEY_WORD]
        else:
            lib.i("`{}` not found in {}, skip!".format(self.modules_key_word, self.ci_config_file))
        lib.i("parse aop config finished {}".format(project_path))
    ...
    def __parse_ci_config_feature_info(self, work_path):
        ...
        :return:
        """
        module = lib.data.arguments.get_opt('-f')
        if module is None:
            return

        lib.i("update feather headers... {}".format(work_path))
        self.work_path = work_path
        # 校验文件
        source_file_path = os.path.join(work_path, self.ci_config_file)
        if not os.path.exists(source_file_path):
            lib.e("{} not found in {}, skip!".format(self.ci_config_file, work_path))
            return
        ...
        try:
            ...
            traceback.print_exc()
            raise lib.TmakeException("parse [{}] error, {}".format(self.ci_config_file, repr(e)))
        lib.v("ci_config_dict = {} ...".format(ci_config_dict))

        if self.modules_key_word not in ci_config_dict:
            lib.TmakeException("`{}` not found in {}, skip!".format(self.modules_key_word, self.ci_config_file))
            return
    ...
    def __generate_headers(self, dir, feature_dict):
        ...
        if not isinstance(feature_dict, dict):
            raise lib.TmakeException("Input param2 must be a dict!!!")
        file_path = ""
        try:
            for key, value in feature_dict.items():
                ...
                f.write(model)
                f.close()
                lib.s("generate header success: {}".format(file_path))
        except:
            lib.TmakeException("generate header failed: {}".format(file_path))
