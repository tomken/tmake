...
from lib import tmake_log
from lib.tmake_exception import TmakeException
...
class VSToolInfo(object):
    ...
    def get_build_tool_by_arch(self, arch):
        if self.version == "vs2017":
            if arch.lower().endswith(lib.TARGET_CPU_X64):
        ...
        from lib.utils import comm_utils
...
class EnvironmentInfo(object):
    ...
    def get_vs_tool_path(self, arch):
        ...
        if not os.path.exists(vs_build_tool):
            lib.e("{} is not exist!".format(vs_build_tool))
            self.__show_help()
